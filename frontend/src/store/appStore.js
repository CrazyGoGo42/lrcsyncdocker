...
export const useAppStore = create(
  persist(
    (set, get) => ({
        ...
        set((state) => {
          ...
          if (nextIndex < state.queue.length) {
            return {
              ...
              currentTime: 0,
              isPlaying: state.isPlaying, // Maintain playing state
            };
      ...
      // Seeking functionality - FIXED to prevent loops
      seekToTime: (time) =>
        set(
          (state) => {
            // Only update if the time is significantly different to prevent loops
            if (Math.abs(state.currentTime - time) > 0.5) {
              return {
                seekTime: time,
                currentTime: time,
              };
